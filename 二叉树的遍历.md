...
**迭代算法**：从栈里弹出的孩子， 不需要在遍历其左孩子。为了防止多次进入右子树多次遍历同一个又孩子， 右孩子回退的时候需要加一个prev指针用来防止进入无限循环。
```
/**
 * Definition for a binary tree node.
 * type TreeNode struct {
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTcyMTc0MzcwMSwxNTUzMTA4NjQ3LDU3ND
MwNTQyMywtNTE4MDE3MzIsLTM2MzA0NjQ1LDkzODI3MTMzNl19
